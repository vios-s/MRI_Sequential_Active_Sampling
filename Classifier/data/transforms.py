...
from .slice_utils import process_kspace, process_recon, to_tensor, normalize_instance, complex_center_crop, normalize
...
class DataTransform:
    ...
    def __call__(self, kspace: np.ndarray, mask: np.ndarray, recon: np.ndarray, max_value: float, fname: str, slice_num: int, label: dict
        ...
        Returns: A tuple containing, 
            ...
            undersampled: normalized undersampled,
            mask: mask,
        ...
        recon = complex_center_crop(ifft2c(tensor_kspace), self.recon_size)
        ...
        recon_complex = torch.view_as_complex(recon)  # (320,320) complex
        abs_recon = torch.abs(recon_complex).unsqueeze(0)  # (1,320,320)

        undersampled_complex = torch.view_as_complex(undersampled)
        abs_undersampled = torch.abs(undersampled_complex).unsqueeze(0)

        _, mean, std = normalize_instance(abs_undersampled, eps=1e-11)
        normalized_recon = normalize(abs_recon, mean, std, eps=1e-11)


        undersampled = undersampled.permute(2, 0, 1)
        normalized_undersampled, mean, std = normalize_instance(undersampled, eps=1e-11)
